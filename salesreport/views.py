...
from salesreport.serializers.serializers import CustomerSegmentsAnalyticsSerializer, RecentActivityAnalyticsSerializer, SalesDataReportSerializer, TopProductsAnalyticsSerializer
...
class AnalyticsSalesReportsAPIView(APIView):
    ...
    def get(self, request):
        ...
        top_products = (
            ...
            .filter(order__status='PROCESSING')
        ...
        prev_sales = (
            OrderProduct.objects
            .filter(order__created_at__month=prev_month)
            .values('product__sku')
            .annotate(prev_sales=Sum('quantity'))
        )

        prev_sales_map = {
            p['product__sku']: p['prev_sales'] for p in prev_sales
        }
        ...
        recurrent_customers = User.objects.annotate(
            orders_count=Count('order')
        ).filter(orders_count__gt=1).count()

        vip_customers = (
            Payment.objects
            .values('order__user__dni')
        ...
        return Response({
            ...
            'top_products': TopProductsAnalyticsSerializer(top_products_data, many=True).data,
            'customer_segments': CustomerSegmentsAnalyticsSerializer(customer_segments_data, many=True).data,
            'recent_activity': RecentActivityAnalyticsSerializer(recent_activity, many=True).data,
